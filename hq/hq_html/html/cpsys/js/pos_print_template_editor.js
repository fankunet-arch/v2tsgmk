...
$(document).ready(function() {
    ...
    const templates = $('#visual-editor-templates'); // Templates are static

    // --- V6.2.7: Editor-Internal Variables ---
    // These are declared 'let' and will be *assigned* only AFTER the offcanvas is shown.
    ...
    /**
     * ===================================================================
     * V6.2.7: NEW INITIALIZATION FUNCTION
     * Initializes all editor variables and binds events.
     * This MUST be called only AFTER the offcanvas is visible.
     * ===================================================================
     */
    function initializeEditor(jsonString) {
        // 1. Assign variables now that DOM elements are guaranteed to exist
        ...
        // 2. Bind (or re-bind) all internal event handlers safely
        // We use delegation from the drawer itself for robustness
    ...
    /**
     * ===================================================================
     * Visual Editor Core Functions (Unchanged from V6.2.4)
     * ===================================================================
     */
    ...
    function initializeSortable() {
        ...
        // Ensure canvas is valid before creating Sortable
        if (!canvas || canvas.length === 0) {
            console.error("Editor canvas not found. Cannot initialize Sortable.");
            return;
        }
    ...
    function renderVisualEditor(jsonString) {
        if (!canvas) {
             console.error("renderVisualEditor called before canvas was initialized.");
             return;
        }
        canvas.empty();
        let items = [];
    ...
    // 6. (REWRITTEN V6.2.2) Update the preview pane
    function updatePreviewPane() {
        if (!previewPaper) return; // Safety check
        ...
        // --- START V6.2.2 REWRITE ---
        const renderItem = (item) => {
            let itemHtml = ''; // This is the string we will build and return
            ...
            switch (item.type) {
                case 'text':
                    itemHtml += `<pre class="preview-align-${item.align} preview-size-${item.size}">${escapeHtml(value)}</pre>`;
                    break;
                case 'kv':
                    itemHtml += `<pre class="preview-kv-row"><span class="preview-kv-key">${escapeHtml(key)}</span><span class="preview-kv-value ${item.bold_value ? 'bold' : ''}">${escapeHtml(value)}</span></pre>`;
                    break;
                case 'divider':
                    itemHtml += `<pre class="preview-align-center">${escapeHtml(String(item.char || '-').repeat(32))}</pre>`;
                    break;
                case 'feed':
                    for(let i=0; i < item.lines; i++) { itemHtml += '<br>'; }
                    break;
                case 'qr_code':
                    itemHtml += `<pre class="preview-align-${item.align}"><span class="preview-qr-code">[二维码: ${escapeHtml(value)}]</span></pre>`;
                    break;
                case 'cut':
                    itemHtml += `<pre class="preview-cut">-- (切纸) --</pre>`;
                    break;
                case 'items_loop':
                    itemHtml += `<div class="preview-loop-box">`;
                    // Render 2 mock items
                    for(let i=0; i < 2; i++) {
                        // ***** START V6.2.4 FIX: Corrected missing '=' in class *****
                        itemHtml += `<div class="preview-loop-item">`;
                        // ***** END V6.2.4 FIX *****
                        
                        if (i > 0) itemHtml += `<pre>${escapeHtml("-".repeat(28))}</pre>`; // separator for 2nd item
            ...
            return itemHtml; // Return the built string
        };
        
        // Build the final HTML by mapping and joining the results
    ...
    // 7. Check state of the 'Add Loop' button
    ...
    /**
     * ===================================================================
     * V6.2.7: MODIFIED GLOBAL EVENT HANDLERS
     * ===================================================================
     */
    ...
    $('#create-btn').on('click', function() {
        ...
        dataDrawerEl.addEventListener('shown.bs.offcanvas', () => {
            // Initialize *after* DOM is visible
            initializeEditor('[]'); 
            // Set default *after* init and trigger change
    ...
    $('.table').on('click', '.edit-btn', function() {
        ...
        // Clean canvas *before* AJAX (if it exists from a previous open)
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    // Set form values *before* opening the drawer
    ...
    dataDrawerEl.addEventListener('hidden.bs.offcanvas', () => {
        ...
        // Detach all editor-specific events to prevent memory leaks
        $('#data-drawer').off('.editor');
        if (physicalSizeSelect) {
            physicalSizeSelect.off('.editor');
        }
