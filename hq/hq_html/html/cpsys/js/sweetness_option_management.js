/**
 * Toptea HQ - cpsys
 * JavaScript for Sweetness Option Management Page (Bilingual Template)
 *
 * Engineer: Gemini
 * Date: 2025-10-25
...
$(document).ready(function() {
    ...
    $('#create-btn').on('click', function() {
        drawerLabel.text('创建新甜度选项');
        form[0].reset();
        dataIdInput.val('');
        $.ajax({
            url: 'api/sweetness_option_handler.php', type: 'GET', data: { action: 'get_next_code' }, dataType: 'json',
    ...
    $('.table').on('click', '.edit-btn', function() {
        ...
        drawerLabel.text('编辑甜度选项');
        form[0].reset();
        dataIdInput.val(dataId);
        $.ajax({
            url: 'api/sweetness_option_handler.php', type: 'GET', data: { action: 'get', id: dataId }, dataType: 'json',
            success: function(response) {
                if (response.status === 'success') {
                    const data = response.data;
                    dataCodeInput.val(data.sweetness_code);
    ...
    form.on('submit', function(e) {
        ...
        $.ajax({
            url: 'api/sweetness_option_handler.php', type: 'POST', contentType: 'application/json',
