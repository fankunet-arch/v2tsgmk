$(function () {
    const API_URL = 'api/pos_promotion_handler.php';
    ...
    function renderConditionParams($container, type, data = {}) { /* ... no changes in this function ... */
        let content = '';
    ...
    function renderActionParams($container, type, data = {}) { /* ... no changes in this function ... */
        let content = '';
    ...
    // CORE CHANGE: Toggle promo code input visibility
    ...
    function addRow(containerSelector, templateSelector, isAction = false) { /* ... no changes in this function ... */
    ...
    function resetForm() {
        ...
        togglePromoCodeInput(); // Ensure correct initial state
    }
    ...
    $('.edit-btn').on('click', function() {
        ...
        $.get(API_URL, { action: 'get', id: promoId }, function(res) {
            if (res.status === 'success') {
                ...
                $('#promo_code').val(p.promo_code); // CORE CHANGE: Populate promo code
                ...
                togglePromoCodeInput(); // Show/hide field based on loaded data

                const conditions = JSON.parse(p.promo_conditions || '[]');
                ...
                const actions = JSON.parse(p.promo_actions || '[]');
                actions.forEach(act => {
    ...
    function buildRules(containerSelector, isAction = false) { /* ... no changes in this function ... */
        const rules = [];
    ...
    $('#promo-form').on('submit', function(e) {
        ...
        const data = {
            ...
            promo_code: $('#promo_code').val(), // CORE CHANGE: Get promo code value
        ...
        $.ajax({
            url: API_URL, type: 'POST', contentType: 'application/json', data: JSON.stringify({ action: 'save', data: data }),
            success: (res) => { if (res.status === 'success') { alert(res.message); location.reload(); } else { alert(res.message); } },
            error: () => alert('保存时发生错误。')
        });
    });

    $('.delete-btn').on('click', function() { /* ... no changes in this function ... */
        const id = $(this).data('id'), name = $(this).data('name');
        if (confirm(`确定要删除活动 "${name}" 吗？此操作不可撤销。`)) {
             $.ajax({
                url: API_URL, type: 'POST', contentType: 'application/json', data: JSON.stringify({ action: 'delete', id: id }),
                success: (res) => { if (res.status === 'success') { alert(res.message); location.reload(); } else { alert(res.message); } },
                error: () => alert('删除时发生错误。')
            });
        }
