 ...
 * [GEMINI 404_FIX]: Restored all missing handlers (users, stores, cups, units, etc.)
 */
...
function handle_store_save(PDO $pdo, array $config, array $input_data): void {
    ...
    $invoice_number_offset  = (string)($s['invoice_number_offset'] ?? '') === '' ? 10000 : (int)$s['invoice_number_offset'];
    ...
    $printer_type = strtoupper(trim((string)($s['printer_type'] ?? 'NONE')));
    if (!in_array($printer_type, $printer_allowed, true)) $printer_type = 'NONE';

    // 打印机细项：当类型为 NONE 时置空，避免无效值写入
    if ($printer_type === 'NONE') {
        $printer_ip   = null;
        $printer_port = null;
        $printer_mac  = null;
    } else {
        $printer_ip   = ($s['printer_ip'] ?? '') === '' ? null : trim((string)$s['printer_ip']);
        $printer_port = ($s['printer_port'] ?? '') === '' ? null : (int)$s['printer_port'];
        $printer_mac  = ($s['printer_mac'] ?? '') === '' ? null : trim((string)$s['printer_mac']);
    }
    ...
    try {
        ...
        if ($id > 0) {
            $sql = "UPDATE kds_stores
                       SET store_code=?, store_name=?, tax_id=?, default_vat_rate=?, invoice_number_offset=?,
                           store_city=?, store_address=?, store_phone=?, store_cif=?, is_active=?,
                           billing_system=?, eod_cutoff_hour=?, printer_type=?, printer_ip=?, printer_port=?, printer_mac=?
                     WHERE id=?";
            $pdo->prepare($sql)->execute([
                $store_code, $store_name, $tax_id, $default_vat_rate, $invoice_number_offset,
