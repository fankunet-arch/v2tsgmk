 ...
 * 注册物料、库存、配方等资源
 * Version: 1.2.3 (unique handler names to avoid collisions)
 * Date: 2025-11-06
 *
 * 关键修复：
 * - 将本文件全部 handler 重命名为 cprms_* 前缀，避免与 kds_repo_*.php 的函数同名。
 * - 将 get_base_quantity 调整为顶层 cprms_get_base_quantity（带 function_exists 保护）。
 * - 不改业务逻辑，不增删 SQL，仅做命名去碰撞。
 */
