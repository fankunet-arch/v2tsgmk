 ...
 * 注册物料、库存、配方等资源
 * Version: 1.2.0 (3-Level-Unit Support)
 * Date: 2025-11-05
 *
 * [GEMINI FIX]: Corrected realpath() to ../../../
 * [GEMINI 3-LEVEL-UNIT FIX]:
 * 1. Renamed large_unit -> medium_unit in handle_material_save.
 * 2. Added large_unit_id and large_conversion_rate logic.
 * 3. Updated validation logic for medium and large units.
 * 4. Updated handle_stock_actions to correctly calculate quantity
 * from base, medium, or large unit ID.
 */
...
function handle_material_save(PDO $pdo, array $config, array $input_data): void {
    ...
    // 单位/换算：0 视为未使用 → 入库用 NULL；使用时必须有 >0 的换算率
    ...
    // 效期规则：'' 代表未设置 → 入库用 NULL；END_OF_DAY 也不需要时长
    ...
    if ($expiry_rule_input === '' || $expiry_rule_input === 'END_OF_DAY') {
        $expiry_rule_type = null;     // ← 关键修复：空字符串改为 NULL，避免 ENUM 报错
        $expiry_duration  = 0;
    } elseif (in_array($expiry_rule_input, ['HOURS','DAYS'], true)) {
        if ($expiry_duration <= 0) {
            json_error('选择按小时或天计算效期后，必须填写一个大于0的时长。', 400);
        }
    ...
    try {
        if ($id) {
            ...
            if ($stmt_check->fetch()) {
                json_error('自定义编号 "' . htmlspecialchars($code) . '" 已被另一个有效物料使用。', 409);
            }
            ...
            $stmt = $pdo->prepare("
                UPDATE kds_materials SET
                    ...
                    base_unit_id  = ?,
                    medium_unit_id = ?,
                    medium_conversion_rate = ?,
                    large_unit_id  = ?,
                    large_conversion_rate  = ?,
                    expiry_rule_type = ?,
                    expiry_duration = ?
                WHERE id = ?
            ");
            ...
            if ($stmt_active->fetch()) {
                json_error('自定义编号 "' . htmlspecialchars($code) . '" 已被一个有效物料使用。', 409);
            }
...
function handle_stock_actions(PDO $pdo, array $config, array $input_data): void {
    ...
    // [MODIFIED] 提取公共的转换逻辑
    function get_base_quantity(PDO $pdo, int $material_id, float $quantity_input, int $unit_id): float {
        if ($material_id <= 0 || $quantity_input <= 0 || $unit_id <= 0) json_error('物料、数量和单位均为必填项。', 400);
        ...
        if ($material['base_unit_id'] == $unit_id) {
            // 基础单位，无需转换
            return $final_quantity;
        } 
        elseif ($material['medium_unit_id'] == $unit_id) {
            // 中级单位 -> 基础单位
            $med_rate = (float)($material['medium_conversion_rate'] ?? 0);
            if ($med_rate <= 0) json_error('该物料的“中级单位”换算率未设置或无效。', 400);
            $final_quantity = $quantity_input * $med_rate;
        } 
        elseif ($material['large_unit_id'] == $unit_id) {
            // 大单位 -> 基础单位
            $med_rate = (float)($material['medium_conversion_rate'] ?? 0);
            $large_rate = (float)($material['large_conversion_rate'] ?? 0);
            if ($med_rate <= 0 || $large_rate <= 0) json_error('该物料的“中级单位”或“大单位”换算率未设置或无效。', 400);
            $final_quantity = $quantity_input * $large_rate * $med_rate; // 大 -> 中 -> 基础
        }
        else {
            // 传入的 unit_id 不属于该物料的任何一个单位
    ...
    if ($action === 'add_warehouse_stock') {
        ...
        // [MODIFIED]
        $final_quantity_to_add = get_base_quantity($pdo, $material_id, $quantity_to_add, $unit_id);
        // [END MOD]
    ...
    elseif ($action === 'allocate_to_store') {
        ...
        // [MODIFIED]
        $final_quantity_to_allocate = get_base_quantity($pdo, $material_id, $quantity_to_allocate, $unit_id);
        // [END MOD]
