...
function handle_order_submit(PDO $pdo, array $config, array $input_data): void {
    ...
    $print_jobs = [];
    // [PHASE 3a] $series 现在是 {Prefix}Y{YY}
    $full_invoice_number = $series . '-' . $invoice_number;
...
function handle_data_load(PDO $pdo, array $config, array $input_data): void {
    ...
    foreach ($results as $row) {
        ...
        $products[$itemId]['variants'][] = [
            'id' => (int)$row['variant_id'], 'recipe_sku' => $row['product_sku'],
            'name_zh' => $row['variant_name_zh'], 'name_es' => $row['variant_name_es'],
            'price_eur' => (float)$row['price_eur'], 'is_default' => (bool)$row['is_default']
        ];
    }
